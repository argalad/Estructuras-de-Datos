...
public:
	...
	Pelicula(string s, Horas h, Horas d) : nombre(s), hora_inicio(h), duracion(d), hora_fin(h+d) {};
...
inline ostream& operator<< (ostream& out, Pelicula const &p) {
	out << setfill('0') << setw(2) << p.get_hora_fin().get_hora() << ":" << setfill('0') << setw(2) << p.get_hora_fin().get_minutos() << ":" << setfill('0') << setw(2) << p.get_hora_fin().get_segundos() << " " << p.get_nombre() << endl;
	return out;
}
