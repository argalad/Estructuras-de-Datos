...
public:
	...
	bool operator== (Horas const& h) const {
		return (this->hora == h.hora && this->minutos == h.minutos && this->segundos == h.segundos);
	}
...
inline ostream& operator<< (ostream& out, Horas& h) {
	out << setfill('0') << setw(2) << h.get_hora() << ":" << setfill('0') << setw(2)
		<< h.get_minutos() << ":" << setfill('0') << setw(2) << h.get_segundos() << endl;
